...
Pool.prototype.auth = function(mechanism) {
  var self = this;
  var args = Array.prototype.slice.call(arguments, 0);
  if (self.id <= 1 && self.inUseConnections.length === 0 && self.__stalling !== 'done') {
    if (!self.__stalling) {
      console.log("stalling", self.options.port, self.id);
    ...
    return;
  } else if (self.__stalling === 'stalling') {
    console.log("done stalling", self.options.port, self.id);
