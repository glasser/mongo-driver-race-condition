...
function attemptReconnect(self) {
  return function() {
    ...
    function _connectHandler(self) {
      return function() {
        ...
        reauthenticate(self, this, function() {
          ...
          self._checkForDuplicateConnections();
    ...
    if (self.reconnectConnection !== null) {
      console.error("leaking reconnectConnection");
      console.trace();
    }
    // Create a connection
...
Pool.prototype.auth = function(mechanism) {
  var self = this;
  var args = Array.prototype.slice.call(arguments, 0);
  if (self.id <= 1 && self.__stalling !== 'done') {
    if (self.inUseConnections.length === 0) {
      if (!self.__stalling) {
        console.log("stalling pool", self.options.port, self.id);
        self.__stalling = 'stalling';
      }
      setTimeout(function() {
        self.auth.apply(self, args);
      }, 1);
      return;
    }
    // We have an inUse connection, which is the race condition we were waiting
    // for.
    if (self.__stalling === 'stalling') {
      console.log("done stalling pool", self.options.port, self.id);
    } else {
      console.log("done stalling pool (without even starting)", self.options.port, self.id);
    }
    self.__stalling = 'done';
  }
...
Pool.prototype._checkForDuplicateConnections = function() {
  var self = this;
  if (self._alreadyWarnedAboutDuplicateConnections) {
    return;
  }

  var idToFields = {};

  function scan(fieldName) {
    var field = self[fieldName];
    for (var i = 0; i < field.length; i++) {
      scanConnection(field[i], fieldName);
    }
  }

  function scanConnection(connection, fieldName) {
    if (!idToFields[connection.id]) {
      idToFields[connection.id] = [];
    }
    idToFields[connection.id].push(fieldName);
  }

  scan('availableConnections');
  scan('inUseConnections');
  scan('connectingConnections');
  scan('nonAuthenticatedConnections');
  if (self.reconnectConnection) {
    scanConnection(self.reconnectConnection, 'reconnectConnection');
  }

  var failed = false;
  var ids = Object.keys(idToFields);
  for (var i = 0; i < ids.length; i++) {
    var id = ids[i];
    var fields = idToFields[id];
    if (fields.length > 1) {
      console.error("duplicate connection detected:", id, fields);
      failed = true;
    }
  }
  if (failed) {
    console.trace("Trace for duplicate connection");
    self._alreadyWarnedAboutDuplicateConnections = true;
  }
}
