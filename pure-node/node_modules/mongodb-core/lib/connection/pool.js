...
var Pool = function(options) {
  ...
  console.log("queue for", this.options.host);
...
function attemptReconnect(self) {
  return function() {
    ...
    // Create a connection
    console.log("new conn 1", self.options.host);
...
Pool.prototype.connect = function() {
  ...
  // Create a connection
  console.log("new conn 2", self.options.host);
...
Pool.prototype.destroy = function(force) {
  var self = this;
  console.log("destroying pool", self.options.host, force);
  console.trace();
  ...
  if(force) {
    ...
    // an error
    console.log("pool destroy queue", self.options.host);
  ...
  function checkStatus() {
    ...
    // } else if (self.queue.length > 0 && !this.reconnectId) {
      ...
      console.log("emptying queue _execute", self.options.host);
      _execute(self)();
      // Set timeout
...
Pool.prototype.write = function(commands, options, cb) {
  ...
  // Always have options
  options = options || {};
  console.log("calling Pool.write", self.options.host, commands, options);

  // Pool was destroyed error out
  if(this.state == DESTROYED || this.state == DESTROYING) {
    console.log("PW destroy", self.options.host);
  ...
  if(this.options.domainsEnabled
    ...
    console.log("domains thing", self.options.host);
  ...
  if(options.monitoring) {
    ...
    console.log("added monitoring, length now", this.queue.length, self.options.host);
  } else {
    this.queue.push(operation);
    console.log("added non-monitoring, length now", this.queue.length, self.options.host);
  }

  console.log("operation is", self.options.host, operation);
  // Attempt to execute the operation
  if(!self.executing) {
    console.log("executing now", self.options.host);
    process.nextTick(function() {
      _execute(self)();
    });
  }
...
function _createConnection(self) {
  ...
  console.log("new conn 3", self.options.host);
    ...
    return function() {
      ...
      reauthenticate(self, _connection, function(err) {
        ...
        if(self.authenticating) {
          ...
          console.log("execute any work waiting", self.options.host);
          _execute(self)();
        }
      });
    }
...
function flushMonitoringOperations(queue) {
  for(var i = 0; i < queue.length; i++) {
    if(queue[i].monitoring) {
      console.log("queue flushing a monitoring operation", self.options.host);
...
function _execute(self) {
  return function() {
    console.log("_execute top", self.options.host);
    if(self.state == DESTROYED) return;
    // Already executing, skip
    if(self.executing) return;
    console.log("now executing", self.options.host);
    ...
    waitForAuth(function() {
      ...
      while(true) {
        ...
        if(self.availableConnections.length == 0) {
          console.log("no available", self.options.host);
        ...
        // No queue break
        if(self.queue.length == 0) {
          console.log("no queue", self.options.host);
          break;
        }
        ...
        if(connection.isConnected()) {
          console.log("connected connection", self.options.host);
          // Get the next work item
          var workItem = self.queue.shift();
          console.log("got work item", self.options.host, workItem);
          console.log("queue now", self.options.host, self.queue.length);
          ...
          console.log("disconnected connection", self.options.host);
          // Remove the disconnected connection
          removeConnection(self, connection);
          // Flush any monitoring operations in the queue, failing fast
          flushMonitoringOperations(self.queue);
    ...
    console.log("done executing", self.options.host);
