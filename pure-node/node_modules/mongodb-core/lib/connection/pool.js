...
var Pool = function(options) {
  ...
  console.log("queue for", this.options.host, this.id);
  console.trace("starting a brand new pool queue");
  this.queue = [];
...
function connectionFailureHandler(self, event) {
  return function(err) {
    ...
    // console.dir(err)
    console.log("pool connection got event", event, err);

    if (this._connectionFailHandled) return;
    console.log("pool connection made it past failhandled");
    this._connectionFailHandled = true;
    // Destroy the connection
    this.destroy();
    console.log("pool connection made it past destroy", self.options.host, self.id, this.id);

    // Remove the connection
    removeConnection(self, this);
    console.log("pool connection made it past remove");
    ...
    console.log("pool connection made it past timeout");

    // No more socket available propegate the event
    if(self.socketCount() == 0) {
      console.log("pool connection no sockets");
      if(self.state != DESTROYED && self.state != DESTROYING) {
    ...
    console.log("pool connection has sockets", self.availableConnections.length, self.inUseConnections.length, self.options.host, self.id);
    console.log("removing socket has ID", this.id);
    self.logAvailable("end of connectionFailureHandler");
...
function attemptReconnect(self) {
  return function() {
    ...
    function _connectHandler(self) {
      return function() {
        ...
        reauthenticate(self, this, function() {
          ...
          self.logAvailable("_connectHandler post-reauth", connection);
    ...
    // Create a connection
    console.log("new conn 1", self.options.host, self.id);
...
function moveConnectionBetween(connection, from, to, self) {
  self.logAvailable("top of MCB");
  console.trace();
  var index = from.indexOf(connection);
  // Move the connection from connecting to available
  if(index != -1) {
    if (to === self.availableConnections) {
      self.logAvailable("MCB", connection);
    }
    from.splice(index, 1);
    to.push(connection);
  }
  self.logAvailable("bottom of MCB");
}
...
function messageHandler(self) {
  return function(message, connection) {
    ...
    console.log("received message from", self.options.host, self.id, connection.id, message);
    ...
    function authenticateStragglers(self, connection, callback) {
      ...
      if(connections.length == 0) {
        ...
        moveConnectionBetween(connection, self.inUseConnections, self.availableConnections, self);
        // Finish
        return callback();
      }
      ...
      for(var i = 0; i < connectionCount; i++) {
        reauthenticate(self, connections[i], function() {
          ...
          if(connectionCount == 0) {
            ...
            for (var i = 0; i < nonAuthenticatedConnections.length; i++) {
              self.logAvailable("nonauth", nonAuthenticatedConnections[i]);
            }
...
Pool.prototype.logAvailable = function (message, adding) {
  var self = this;
  console.log("Available connections for", self.options.host, self.id, "at time", message);
  for (var i = 0; i < self.availableConnections.length; i++) {
    console.log(i, ": ", self.availableConnections[i].id);
  }
  if (adding) {
    console.log("Going to add", adding.id);
    for (var i = 0; i < self.availableConnections.length; i++) {
      if (adding.id === self.availableConnections[i].id) {
        console.log("ID already found at slot", i);
        console.log("Identical?", adding === self.availableConnections[i]);
        console.trace();
      }
    }
  }
...
Pool.prototype.connect = function() {
  ...
  // Create a connection
  console.log("new conn 2", self.options.host, self.id);
  var connection = new Connection(messageHandler(self), this.options);
  console.log("new conn 2 made", self.options.host, self.id, connection.id);
  ...
  connection.once('connect', function(connection) {
    ...
    if(self.options.inTopology) {
      ...
      moveConnectionBetween(connection, self.connectingConnections, self.availableConnections, self);
...
Pool.prototype.auth = function(mechanism) {
  var self = this;
  console.log("it's auth time", self.options.host, self.id);
  console.trace();
  var args = Array.prototype.slice.call(arguments, 0);
  ...
  function authenticateLiveConnections(self, args, cb) {
    ...
    console.log("clearing for authenticateLiveConnections", self.options.host, self.id);
    ...
    for(var i = 0; i < connections.length; i++) {
      authenticate(self, args, connections[i], function(err) {
        ...
        if(connectionsCount == 0) {
          ...
          for (var i = 0; i < connections.length; i++) {
            self.logAvailable("ALC", connections[i]);
          }
...
Pool.prototype.destroy = function(force) {
  var self = this;
  console.log("I AM POOL DESTROY", self.options.host, self.id, force);
  console.trace();
  ...
  if(force) {
    ...
    // an error
    console.log("pool destroy queue", self.options.host, self.id, self.queue.length);
    while(self.queue.length > 0) {
      var workItem = self.queue.shift();
      if(typeof workItem.cb == 'function') {
        console.log("calling destroy cb", self.options.host, self.id);
  ...
  function checkStatus() {
    ...
    // } else if (self.queue.length > 0 && !this.reconnectId) {
      ...
      console.log("emptying queue _execute", self.options.host, self.id);
      _execute(self)();
      // Set timeout
...
Pool.prototype.write = function(commands, options, cb) {
  ...
  // Always have options
  options = options || {};
  console.log("calling Pool.write", self.options.host, self.id, commands, options);

  // Pool was destroyed error out
  if(this.state == DESTROYED || this.state == DESTROYING) {
    console.log("PW destroy", self.options.host, self.id);
  ...
  if(this.options.domainsEnabled
    ...
    console.log("domains thing", self.options.host, self.id);
  ...
  if(options.monitoring) {
    ...
    console.log("added monitoring, length now", this.queue.length, self.options.host, self.id);
  } else {
    this.queue.push(operation);
    console.log("added non-monitoring, length now", this.queue.length, self.options.host, self.id);
  }

  console.log("operation is", self.options.host, self.id, operation);
  // Attempt to execute the operation
  if(!self.executing) {
    console.log("executing now", self.options.host, self.id);
    process.nextTick(function() {
      _execute(self)();
    });
  }
...
function removeConnection(self, connection) {
  ...
  try {
    ...
    if(remove(connection, self.nonAuthenticatedConnections)) return;
      ...
      self.logAvailable("bottom of RC");
  }
}
...
function _createConnection(self) {
  ...
  console.log("new conn 3", self.options.host, self.id);
    ...
    return function() {
      ...
      reauthenticate(self, _connection, function(err) {
        ...
        if(self.authenticating) {
          ...
          // Push to available
          self.logAvail("tempConnect", _connection);
          self.availableConnections.push(_connection);
          // Execute any work waiting
          console.log("execute any work waiting", self.options.host, self.id);
          _execute(self)();
        }
      });
    }
...
function flushMonitoringOperations(queue) {
  for(var i = 0; i < queue.length; i++) {
    if(queue[i].monitoring) {
      console.log("queue flushing a monitoring operation");
...
function _execute(self) {
  return function() {
    console.log("_execute top", self.options.host, self.id);
    if(self.state == DESTROYED) return;
    // Already executing, skip
    if(self.executing) return;
    console.log("now executing", self.options.host, self.id);
    ...
    waitForAuth(function() {
      console.log("waitForAuth done", self.options.host, self.id);

      // New pool connections are in progress, wait them to finish
      // before executing any more operation to ensure distribution of
      // operations
      if(self.connectingConnections.length > 0) {
        console.log("connecting connections", self.options.host, self.id);
        return;
      }
      ...
      while(true) {
        ...
        if(self.availableConnections.length == 0) {
          console.log("no available", self.options.host, self.id);
        ...
        // No queue break
        if(self.queue.length == 0) {
          console.log("no queue", self.options.host, self.id);
          break;
        }
        ...
        if(connection.isConnected()) {
          console.log("connected connection", self.options.host, self.id);
          // Get the next work item
          var workItem = self.queue.shift();
          console.log("got work item", self.options.host, self.id, workItem);
          console.log("queue now", self.options.host, self.id, self.queue.length);
          ...
          if (workItem.monitoring) {
            moveConnectionBetween(connection, self.availableConnections, self.inUseConnections, self);
          }
          ...
          console.log("disconnected connection", self.options.host, self.id);
          // Remove the disconnected connection
          removeConnection(self, connection);
          // Flush any monitoring operations in the queue, failing fast
          flushMonitoringOperations(self.queue);
    ...
    console.log("done executing", self.options.host, self.id);
