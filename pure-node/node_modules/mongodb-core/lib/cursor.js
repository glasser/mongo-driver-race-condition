...
var nextFunction = function(self, callback) {
  ...
  if(!self.cursorState.init) {
    ...
    if(!self.topology.isConnected(self.options) && self.disconnectHandler != null) {
      ...
      console.log("save for later 1");
      return self.disconnectHandler.addObjectAndMethod('cursor', self, 'next', [callback], callback);
    }
    console.log("couldn't save for later, discon hasHandler", !self.topology.isConnected(self.options), self.disconnectHandler != null);

    try {
      console.log("trying to get server");
      self.server = self.topology.getServer(self.options);
      console.log("got server", self.server);
    } catch(err) {
      // Handle the error and add object to next method call
      if(self.disconnectHandler != null) {
        console.log("save for later 2");
        return self.disconnectHandler.addObjectAndMethod('cursor', self, 'next', [callback], callback);
      }

      console.log("couldn't save for later 2");
  ...
  if(self.cursorState.cursorId == null) {
    ...
    if(isConnectionDead(self, callback)) {
      console.log("got isConnectionDead");
      return;
    }

    // Check if topology is destroyed
    if(self.topology.isDestroyed()) {
      console.log("got destroyed topology");
      return callback(new MongoError('connection destroyed, not possible to instantiate cursor'));
    }

    // query, cmd, options, cursorState, callback
    console.log("calling _find");
    self._find(function(err) {
      console.log("back from _find");
      if(err) {
        console.log("_find got err", err);
      ...
      if(self.cursorState.documents.length == 0
        ...
        console.log("setting notified");
        return setCursorNotified(self, callback);
      }

      console.log("more nextFunction");
